...
## Benchmark results
[//]: # (kam: ppdata/ppdata/pprldmany_05D_noiselessall.svg)
[//]: # (kac: data/ppdata/pprldmany_05D_noiselessall.svg)
![](data/ppdata/pprldmany_05D_noiselessall.svg)

As we can see in the summary above most of the parameter configurations gave similar results except configuration $b3$ which stands for $\lambda=4n,\ \mu=\frac{\lambda}{2}$. It has doubled the offspring number in comparison with the default configuration, which is $b4$ and has the pamameters of $\lambda=4n,\mu=\frac{\lambda}{2}$ and $H=6+3\sqrt{n}$. Slightly better results were also observed with configurations labeled: $b6(\lambda=4n,H=6+3n)$ and $b5(\lambda=4n,H=6+6\sqrt{n})$. It seems the the population size of $4n$ is a rather good choice as configurations with this parameter were almost always on top, with one exception being $b1$ which performbed badly most likley due the the very small number of offspring choosen in the selection phase of the algorithm. 

### Best configuration results
[//]: # (kam: ppdata/2compare/ppdata/des_b3_08cze2101/pprldistr_05D_noiselessall.svg)
[//]: # (kac: data/ppdata/des_b3_08cze2101/pprldistr_05D_noiselessall.svg)
![](data/ppdata/des_b3_08cze2101/pprldistr_05D_noiselessall.svg)
As we can see in the summary above most of the parameters configurations gave similar results except configuration $b3$ which stands for $\lambda=4n,\ \mu=\lambda/2$. It has doubled offspring number in comparison with default configuration. Also better than default were configurations labeled: $b6$ and $b5$ both of which had larger time horizon than our reference parameters set.

Algorithm performs well at ill conditioned functions reaching all targets.

![](plots/pprldistr_05D_lcond.svg)  
...
The DES algorithm was sucessfully implemented, various parameters were tested using the BBOB optimization framework benchmark, and lastly the implementation was packaged into an R library for ease of use. As we can see from the benchmark results the DES algoritm prormed best for the parameter set of $\lambda=4n,\mu=\frac{\lambda}{4}$ and $H=6+3\sqrt{n}(b3)$. We can also identify that the DES worked best for function #? //TODO ktore? and worst for function #? //TODO ktore?.
